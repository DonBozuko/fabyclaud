...
export const obterCapacidades = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    try {
      ...
      console.warn(
        "[FabyClaud] Leitura das chaves na nuvem falhou; usando armazenamento local.",
        erro,
      );
    }
...
export const criarProvedorCustom = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      if (error) {
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
        await supabaseAdmin.from("provedores_custom").insert({
          user_id: context.userId,
          slug,
          nome: data.nome,
          url: data.url,
...
export const apagarLicao = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      const db = context.supabase as unknown as import("./faby/escola.server").Db;
      await db.from("licoes").delete().eq("id", data.id).eq("user_id", context.userId);
    } catch {
      // ignore
    }
    return { ok: true };
...
export const apagarAgente = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
      await supabaseAdmin.from("agentes").delete().eq("id", data.id).eq("user_id", context.userId);
    } catch {
      // ignore
    }
    return { ok: true };
...
export const restaurarBackup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    let b: any = null;
    try {
      const { data: resB } = await context.supabase
        .from("backups")
      ...
      if (!b) {
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
        const { data: adminB } = await supabaseAdmin
          .from("backups")
          .select("projeto_id, arquivos")
          .eq("id", data.id)
          .eq("user_id", context.userId)
    ...
    try {
      const { data: p } = await context.supabase
        ...
        .eq("id", b.projeto_id)
        .maybeSingle();
      atual = (p?.arquivos as Record<string, string>) ?? {};
    } catch {
      // ignore
    }
...
export const apagarBackup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      await context.supabase.from("backups").delete().eq("id", data.id);
    } catch {
      // ignore
    }
    try {
  ...
  .handler(async ({ data, context }) => {
    ...
    if (agenteId.startsWith("pronto:")) {
      ...
      try {
        ...
        if (!instrucoesAgente) {
          const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
          const { data: adminAg } = await supabaseAdmin
            .from("agentes")
            .select("instrucoes")
            .eq("id", agenteId)
            .in("user_id", targetUserIds)
    ...
    if (!projetoId) {
      ...
      try {
        const { data: criado, error } = await context.supabase
          .from("projetos")
          .insert({ id: novoId, user_id: context.userId, nome, modelo: data.model, arquivos: {} })
          .select("id")
          .single();
        if (!error && criado?.id) {
          projetoId = criado.id;
      ...
      if (!criadoOk) {
        try {
          const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
          const { data: adminCriado, error: adminErr } = await supabaseAdmin
            .from("projetos")
            .insert({ id: novoId, user_id: context.userId, nome, modelo: data.model, arquivos: {} })
            .select("id")
            .single();
          if (!adminErr && adminCriado?.id) {
            projetoId = adminCriado.id;
            criadoOk = true;
          }
        } catch {
          // ignore
    ...
    try {
      const { data: h, error: errH } = await context.supabase
        .from("mensagens")
        .select("role, conteudo")
        .eq("projeto_id", projetoId)
        .order("created_at", { ascending: true });
      if (!errH && h && h.length > 0) {
        historicoRows = h;
      } else {
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
        const { data: adminH } = await supabaseAdmin
          .from("mensagens")
          .select("role, conteudo")
          .eq("projeto_id", projetoId)
          .order("created_at", { ascending: true });
        if (adminH && adminH.length > 0) historicoRows = adminH;
      }
    } catch {
      // ignore
    }
    ...
    if (ok) {
      ...
      if (Object.keys(extraido.arquivos).length) {
        ...
        if (exigeArquivos) {
          ...
          try {
            const { error: errModel } = await context.supabase
              .from("projetos")
