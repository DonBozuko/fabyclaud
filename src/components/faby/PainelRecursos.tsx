...
function Agentes({
  ...
  const lista = [
    ...AGENTES_PRONTOS,
    ...(meus.data ?? []).map((a) => ({ ...a, pronto: false as const })),
  ];
...
function Prompts({ onUsar }: { onUsar: (texto: string) => void }) {
  ...
  const lista = [
    ...PROMPTS_PRO,
    ...(meus.data ?? []).map((p) => ({ ...p, pronto: false as const })),
  ];
  ...
  return (
    <div className="space-y-3">
      <button
        ...
        className={botao}
        onClick={async () => {
          const r = await criar({ data: { projeto_id: projetoId!, rotulo: "Cópia manual" } });
        ...
        {(lista.data ?? []).map((b) => (
          ...
          >
            ...
            <button
              ...
              className={botaoLeve}
              onClick={async () => {
                const r = await restaurar({ data: { id: b.id } });
                if (!r.ok) toast.error(r.msg ?? "Não deu.");
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={botao}
          onClick={async () => {
            const r = await salvar({ data: { projeto_id: projetoId, nome: ativo, conteudo } });
            if (!r.ok) toast.error(r.msg ?? "Não deu.");
